CREATE OR REPLACE VIEW compound.view_liquidate_borrow
    AS SELECT CASE WHEN t.symbol
    = 'WETH' THEN 'ETH' ELSE t.symbol END AS token_symbol,
    repay."repayAmount" ::double precision
        / (10 ::double precision
           ^ t.decimals::double precision) AS repay_amount,
    repay."repayAmount" ::double precision
        / (10 ::double precision ^ t.decimals::double precision)
        * p.price AS repay_amount_usd,
    c.symbol AS ctoken_symbol,
    events."seizeTokens" / (10 ::numeric ^ c.decimals) AS seize_ctokens,
    events.liquidator, events.borrower,
    ...
    FROM (SELECT "cErc20_evt_LiquidateBorrow".liquidator,
          ...
          "cErc20_evt_LiquidateBorrow".evt_block_time FROM compound_v2
              ."cErc20_evt_LiquidateBorrow" UNION SELECT
               "cEther_evt_LiquidateBorrow".liquidator,
          ...
          "cEther_evt_LiquidateBorrow".evt_block_time FROM compound_v2
              ."cEther_evt_LiquidateBorrow" UNION SELECT
               "CErc20Delegator_evt_LiquidateBorrow".liquidator,
          ...
          "CErc20Delegator_evt_LiquidateBorrow".evt_block_time FROM
              compound_v2."CErc20Delegator_evt_LiquidateBorrow") events LEFT
    JOIN (SELECT "cErc20_evt_RepayBorrow".payer,
          ...
          "cErc20_evt_RepayBorrow".evt_block_time FROM compound_v2
              ."cErc20_evt_RepayBorrow" UNION SELECT
               "cEther_evt_RepayBorrow".payer,
          ...
          "cEther_evt_RepayBorrow".evt_block_time FROM compound_v2
              ."cEther_evt_RepayBorrow" UNION SELECT
               "CErc20Delegator_evt_RepayBorrow".payer,
          ...
          "CErc20Delegator_evt_RepayBorrow".evt_block_time FROM
              compound_v2."CErc20Delegator_evt_RepayBorrow") repay ON events
        .evt_tx_hash = repay.evt_tx_hash LEFT JOIN compound
                           .view_ctokens c ON events."cTokenCollateral"
    = c.contract_address LEFT JOIN compound.view_ctokens c_repay ON repay
          .contract_address = c_repay.contract_address LEFT JOIN erc20
                                  .tokens t ON c_repay
                                  .underlying_token_address = t.contract_address
                                                              LEFT JOIN prices
                                                                  .usd p ON
                                                              p.minute = date_trunc (
                                                                             'minute',
