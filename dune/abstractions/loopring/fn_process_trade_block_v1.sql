...
CREATE TYPE loopring.trade_struct AS (block_timestamp timestamptz,
                                      ...
                                      accountA integer, accountB integer,
                                      protocolFeeTakerBips double precision,
                                      protocolFeeMakerBips double precision);

CREATE OR REPLACE FUNCTION loopring
    .fn_process_trade_block_v1 (blockSize integer, data bytea,
                                block_timestamp timestamptz)
        RETURNS SETOF loopring.trade_struct AS $$ DECLARE offsetTokens integer;
offsetFills integer;
...
RETURN query SELECT block_timestamp,
    ...
    (get_byte (substr (data, offsetAccounts + i * 5, 5), 0) * 65536
     + get_byte (substr (data, offsetAccounts + i * 5, 5), 1) * 256
     + get_byte (substr (data, offsetAccounts + i * 5, 5), 2))
        / 16,
    (get_byte (substr (data, offsetAccounts + i * 5, 5), 2) * 65536
     + get_byte (substr (data, offsetAccounts + i * 5, 5), 3) * 256
     + get_byte (substr (data, offsetAccounts + i * 5, 5), 4))
        & (1048576 - 1),
    protocolFeeTakerBips,
    protocolFeeMakerBips FROM generate_series (0, blockSize - 1) i;
END;
$$ LANGUAGE PLPGSQL;
