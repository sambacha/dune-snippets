...
SELECT
  block_time,
  AVG(hashrate) OVER (
    ORDER BY block_time ROWS 100 PRECEDING) AS sma_hashrate_100_blocks,
  AVG(block_frequency) OVER (
    ORDER BY block_time ROWS 100 PRECEDING) AS sma_frequency_100_blocks
FROM
  time_and_hash
ORDER BY block_time;
